...
#define TX_BD_NUM		1024
#define RX_BD_NUM		1024
...
/**
 * axienet_dma_management - Management of the multiple channel DMA
 * @ndev:
 *
 * return
 *
 * this function is for meshsr's multi channel dma through single channel 
 *
 */
static struct net_device *local_ndev_list[4];
static struct device_node *local_np[4];
static struct axienet_local *local_lp[4];
...
static int axienet_probe(struct platform_device *pdev)
{
	...
	if (NULL != of_find_property(lp->dev->of_node, "meshsr,dma_channel", NULL)) {
		...
		if (dma_channel_number == 0) {
			...
			np = local_np[0];
			lp->dma_regs = local_lp[0]->dma_regs;
			lp->rx_irq = local_lp[0]->rx_irq;
			lp->tx_irq = local_lp[0]->tx_irq;
			of_node_put(np);
		}
